...
def merge_embeddings(frame_embeddings, ocr_embeddings, df):
    ...
    df["img_embedding"] = df["random_frame"].map(frame_embeddings)
    df["text_embedding"] = df["random_frame"].map(ocr_embeddings)
    df_clean = df.dropna(subset=["img_embedding", "text_embedding", "Frame_hate"])
    merged_df = df_clean[
        ["random_frame", "img_embedding", "text_embedding", "Frame_hate"]
    ].copy()
    merged_df.rename(
        columns={"random_frame": "frameID", "Frame_hate": "label"}, inplace=True
    )
    return merged_df
...
def main():
    ...
    hateMM_dir = "./input/video"
    #df = pd.read_csv(os.path.join(hateMM_dir, "processed_hatemm.csv"))
    df = pd.read_csv("filtered_all_combined.csv")
    ...
    final_df.to_pickle(os.path.join(hateMM_dir, "embeddings_hatemm_mert.pkl"))
